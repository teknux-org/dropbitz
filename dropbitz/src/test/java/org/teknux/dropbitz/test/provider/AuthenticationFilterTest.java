...
import org.teknux.dropbitz.controller.LoginController;
...
public class AuthenticationFilterTest {
	...
	private void setup(String ctxPath, String url, Boolean isAuthorized) {
		...
		when(session.getAttribute(AuthenticationHelper.SESSION_ATTRIBUTE_USER)).thenReturn(isAuthorized ? "FAKE_USER" : null);
	}
	...
	public void testNotAuthenticated() throws IOException {
		...
		verify(session).setAttribute(LoginController.SESSION_ATTRIBUTE_ERROR_MESSAGE, AuthenticationFilter.FORBIDDEN_ERROR_MESSAGE);
	}
	...
	public void testAuthenticated() throws IOException {
		...
		verify(session, never()).setAttribute(LoginController.SESSION_ATTRIBUTE_ERROR_MESSAGE, AuthenticationFilter.FORBIDDEN_ERROR_MESSAGE);
	}
}
